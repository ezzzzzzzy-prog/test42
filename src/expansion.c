...
static int agrandir(char **res, size_t *size) // c'est une fonction pour
                                              // agrandir
{
    *size *= 2;
...
static int copy(const char *val, char **res, size_t *j,
                size_t *size) // pour copier
{
    while (*val)
    {
        if (*j >= *size - 1)
...
static int expand_special(struct special *spe, char spe_char, char **res,
                          size_t *j, size_t *size)
{
    if (spe && spe->args)
        return 1;
...
static int expand_braced_var(struct parser *parser, const char *word, size_t *i,
...
static int expand_simple_var(struct parser *parser, const char *word, size_t *i,
                             char **res, size_t *j, size_t *size)
{
...
static int expand_double_quot(const char *word, size_t *i, char **res,
                              size_t *j, size_t *size)
{
    (*i)++;
    char n = word[*i];
...
char *expand(struct parser *parser, struct special *spe, const char *word)
{
    ...
    while (word[i])
    {
        if (word[i] == '\\')
        {
            if (!expand_double_quot(word, &i, &res, &j, &size))
            {
                free(res);
                return NULL;
            }

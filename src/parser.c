...
struct ast *parse_simple_command(struct parser *parser)
{
	...
	int count = 0;
	int capacity = 10;
	while(parser->curr_tok && parser->curr_token->type == TOK_WORD)
	{
		if(count >= capacity)
		{
			capacity *= 2;
...
struct ast *parse_compound_list(struct parser *parser)
{
	...
	while (parser->curr_tok && parser->curr_tok->type == TOK_SEMI || parser->curr_tok->type == TOK_NEWLINE)
	{
		...
		if(count >= capacity)
                {
                        capacity *= 2;
                        char **cm = realloc(commands, capacity *sizeof(char *));
                        if(!cm)
                        {
                                for(int i = 0; i < count; i++)
                                {
	...
	while(parser->curr_tok && parser->curr_tok->type == TOK_NNEWLINE)
	{
		free_tok(parser->curr_tok);
		parser->curr_tok = pop(parser->lex);
	}

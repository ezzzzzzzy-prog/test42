...
struct ast *parse_simple_command(struct parser *parser)
{
  ...
  /*  while (1)
    {
        // Vérifie AVANT de faire quoi que ce soit
        ...
        // Consomme (libère le token et en prend un nouveau)
...
/*struct ast *parse_simple_command(struct parser *parser)
{
    ...
    struct token *curr = parser->curr_tok;
